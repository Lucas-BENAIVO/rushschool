  ...
  return (
    <div className="relative h-full overflow-hidden rounded-xl bg-gradient-to-br from-[#f3e4e2] to-[#e8d4d2] shadow-[0_4px_20px_rgba(0,0,0,0.06)]">
      <div className="relative aspect-[16/10] w-full lg:absolute lg:inset-0 lg:aspect-auto lg:min-h-[220px]">
        ...
        {/* Avant — même échelle, masqué par le clip */}
        <div
          className="absolute inset-y-0 left-0 overflow-hidden"
          style={{ width: `${position}%` }}
        >
          <div
            className="relative h-full max-w-none"
            style={{ width: beforeLayerWidth }}
          >
            {beforeSrc ? (
              <Image
                src={beforeSrc}
              ...
              <div className="flex h-full w-full items-center justify-center bg-[#e8ddd9] text-[11px] text-[var(--kba-muted)]">
                {beforeImageFile}
              </div>
            )}
          </div>
        ...
        <span className="pointer-events-none absolute left-3 top-3 z-20 rounded bg-black/50 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-white">
          Avant
        </span>
        <span className="pointer-events-none absolute right-3 top-3 z-20 rounded bg-black/50 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-white">
          Après
        </span>
      </div>
